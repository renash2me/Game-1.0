...
func _load_inventory() -> void:
	ApiClient.get_req("/api/inventory/" + _char_id, func(code: int, data):
...
func _select_item(idx: int) -> void:
	...
	_detail_info.text = (
		...
		"Equipado: %s" % ("Sim" if item.get("is_equipped", false) else "Não")
	)
